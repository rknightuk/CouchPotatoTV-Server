...
var Presenter = {
    ...
    menuBarItemPresenter: function(xml, ele) {
        ...
        */
        ...
        var searchField = doc.getElementsByTagName("searchField");
        if (searchField.length > 0) {
            searchField = searchField.item(0);
    ...
    load: function(event) {
        ...
        var self = this,
            ...
            var addToWanted = ele.getAttribute("addToWanted");
            var markAsDone = ele.getAttribute("markAsDone");

            if (addToWanted && event.type == 'select') {
                confirmAdd(addToWanted);
            }

            if (markAsDone && event.type == 'select') {
                confirmMarkAsDone(markAsDone, ele);
            }
